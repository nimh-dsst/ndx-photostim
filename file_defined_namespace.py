...
slm = NWBGroupSpec(name='slm', neurodata_type_def='SpatialLightModulator', neurodata_type_inc='Device',
                   doc='slm',
                   attributes=[
                       NWBAttributeSpec('dimension', 'dimension', 'numeric', shape=((2,), (3,)), required=False)], quantity='?')
...
mask_roi = NWBDatasetSpec(doc='ROI masks for each ROI. Each image mask is the size of the original imaging plane (or'
                              'volume) and members of the ROI are finite non-zero.', name='mask_roi', quantity='?',
...
sp = NWBGroupSpec(neurodata_type_def='StimulusPresentation', neurodata_type_inc='TimeIntervals',
                  ...
                  datasets=[label_col, description_col,  stim_col],
